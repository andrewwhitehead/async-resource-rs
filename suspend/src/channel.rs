...
const STATE_LOCKED_LISTEN: u8 = 0b111;
...
impl<T> Channel<T> {
    ...
    pub fn cancel_recv(&mut self) -> bool {
        match self.state.swap(STATE_IDLE, Ordering::AcqRel) {
            STATE_IDLE => true,
            STATE_LISTEN | STATE_LOCKED_LISTEN => {
    ...
    pub fn cancel_send(&mut self) -> bool {
        self.locked_notify() == (false, false)
    }
        ...
        let ready = match self.state.load(Ordering::Acquire) {
            ...
            STATE_LISTEN => {
                ...
                match self.state.compare_exchange(
                    ...
                    Err(STATE_IDLE) | Err(STATE_LOCKED) => {
            ...
            STATE_LOCKED_WAIT | STATE_LOCKED_LISTEN => {
    ...
    fn lock_and_clear_waiting(&mut self) -> (bool, bool) {
        ...
        let cleared = match prev & STATE_LISTEN {
            STATE_IDLE => false,
            STATE_WAIT => true,
            STATE_LISTEN => {
                unsafe { self.drop_waker() };
    ...
    fn lock(&mut self) -> (bool, bool) {
        ...
        (prev & STATE_LOCKED == 0, prev & STATE_LISTEN != 0)
    }
    ...
    fn locked_notify(&self) -> (bool, bool) {
